...
export class GithubSyncService {
  ...
  constructor(private prisma: PrismaService) {
    ...
    if (!token) {
      this.logger.warn('GITHUB_TOKEN não configurado. As requisições podem sofrer rate limit.');
    }
  }
  ...
  async syncClosedPRs(projectId?: string) {
    ...
    const projects = await this.prisma.project.findMany({ where: whereClause as any });

    if (projects.length === 0) {
      this.logger.log('Nenhum projeto configurado para sincronização com GitHub.');
    ...
    for (const project of projects) {
      this.logger.log(`Consultando PRs para ${project.githubOwner}/${project.githubRepo}`);
      ...
      try {
          ...
          if (matches) {
            for (const filename of matches) {
              ...
              if (task && task.status !== 'done') {
                ...
                this.logger.log(`Task ${filename} (ID: ${task.id}) marcada como done através do PR #${pr.number}`);
                totalUpdated++;
              }
            }
          }
        ...
        this.logger.error(`Erro ao buscar dados do repositório ${project.githubOwner}/${project.githubRepo}`, error);
      }
    }
