...
export class GithubSyncCron {
  ...
  async handleCron() {
    this.logger.debug(
      'Iniciando CRON job automático para sincronização do GitHub...',
    );
    try {
      ...
      if (result.totalAtualizados !== undefined) {
        this.logger.debug(
          `CRON job finalizado. Tasks atualizadas: ${result.totalAtualizados}`,
        );
      }
    } catch (error) {
      this.logger.error(
        'Erro ao executar o CRON job de sincronização do GitHub',
        error,
      );
    }
  }
