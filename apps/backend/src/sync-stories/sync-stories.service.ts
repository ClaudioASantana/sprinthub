...
export class SyncStoriesService {
  ...
  async syncFromMarkdown() {
    ...
    for (const file of files) {
      ...
      const title = data.title || this.extractTitleFromContent(content) || file.replace('.md', '');
      const projectId = data.projectId;
      
      if (!projectId) {
        this.logger.warn(`Arquivo ${file} ignorado pois não possui projectId no frontmatter.`);
        continue;
      }
